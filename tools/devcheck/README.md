...
```powershell
# 仓库根目录
pwsh tools/devcheck/devcheck.ps1                    # all：vendor ps1 packaging host contract hosttest ci
...
| `ui` | `src/Ui` 的 ESLint、Vitest 与 `vite build`（**不在 `all` 里**，要先 `cd src/Ui && npm install`） | node + npm | 视机器 |
